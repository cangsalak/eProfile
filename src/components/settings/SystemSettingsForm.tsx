        ...
        >
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              {(() => {
                const activeColor = settings.systemColor || 'indigo';
                return [
                  ...
                  { id: 'custom', name: 'สีแต่งเอง', color: 'bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500' },
                ].map(c => {
                  const isSelected = activeColor === c.id;
                  return (
                    <label key={c.id} className={`cursor-pointer border-2 rounded-2xl p-3 flex flex-col items-center justify-center gap-2 transition-all ${isSelected ? 'border-primary-500 bg-primary-500/10 shadow-md ring-2 ring-primary-500/20' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 hover:border-slate-300 dark:hover:border-slate-700'}`}>
                      <input type="radio" name="systemColor" value={c.id} checked={isSelected} onChange={handleChange} className="sr-only" />
                      <div className={`w-8 h-8 rounded-full ${c.color} shadow-md flex items-center justify-center text-white text-xs`}>
            ...
            {(settings.systemColor === 'custom' || (!settings.systemColor && false)) && (
