...
export async function POST(req: Request) {
  try {
    // 1. One-time Check: Ensure system is not already installed
    ...
    if (isInstalledSetting?.value === 'true' && adminCount > 0) {
      return NextResponse.json({ error: 'ระบบได้รับการติดตั้งเรียบร้อยแล้ว (System is already installed)' }, { status: 403 });
    }
    ...
    // 2. Secret Verification: Validate ADMIN_SETUP_SECRET if configured
    ...
    try {
      ...
      const cmd = fs.existsSync(prismaBin)
        ? `node "${prismaBin}" db push --schema="${schemaFile}" --skip-generate`
        : `npx prisma db push --schema="${schemaFile}" --skip-generate`;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const superAdmin = await tx.personnel.upsert({
        ...
        update: {
          ...
          department: department || 'กองบังคับการ',
      ...
      // 5.5 Seed Departments or Demo Dataset
      if (installDemoData) {
        await seedDemoDataset(tx as any);
      } else {
        // Clean Installation: Create user-specified initial departments or default primary departments
        const deptsToCreate = initialDepartments && initialDepartments.trim()
          ? initialDepartments.split(/[,;\n]/).map(d => d.trim()).filter(Boolean)
          : [department || 'กองบังคับการ', 'ฝ่ายอำนวยการ'];

        for (let i = 0; i < deptsToCreate.length; i++) {
          const dName = deptsToCreate[i];
