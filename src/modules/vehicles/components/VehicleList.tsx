...
export default function VehicleList({ personnelId: initialPersonnelId }: VehicleListProps) {
  const [resolvedPersonnelId, setResolvedPersonnelId] = useState<string>(initialPersonnelId || '');
  ...
  useEffect(() => {
    if (!initialPersonnelId) {
      fetch('/api/auth/me')
        .then(r => r.json())
        .then(data => {
          if (data?.user?.id) setResolvedPersonnelId(data.user.id);
        })
        .catch(() => {});
    } else {
      setResolvedPersonnelId(initialPersonnelId);
    }
  }, [initialPersonnelId]);

  const fetchVehicles = useCallback(async () => {
    if (!resolvedPersonnelId) return;
    try {
      setLoading(true);
      const res = await fetch(`/api/vehicles?personnelId=${resolvedPersonnelId}`);
      if (res.ok) {
  ...
  }, [resolvedPersonnelId, fetchVehicles]);
  ...
  return (
    <div className="mt-8">
      ...
      <ConfirmModal
        ...
        message={`คุณต้องการลบข้อมูลรถทะเบียน "${deleteVehicle?.licensePlate}" ใช่หรือไม่? ข้อมูลที่ลบจะไม่สามารถกู้คืนได้`}
        confirmText="ยืนยันการลบ"
        cancelText="ยกเลิก"
        isDestructive={true}
        onConfirm={() => {
          if (deleteVehicle) {
            handleDeleteVehicle(deleteVehicle.id);
