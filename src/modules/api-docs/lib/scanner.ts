    ...
    for (const [endpoint, methodMap] of Object.entries(API_CATALOGUE_METADATA)) {
      for (const [method, metadata] of Object.entries(methodMap)) {
        ...
        apis.push({
          ...
          endpoint,
          method: method as any,
          category: metadata.category || deriveCategoryFromEndpoint(endpoint),
          description: metadata.description || `API สำหรับ ${endpoint} (${method})`,
          purpose: metadata.purpose || `จัดการข้อมูลผ่าน Endpoint ${endpoint}`,
          authRequired: !isPublic,
          authGuard: isPublic ? 'Public API (ไม่มีการตรวจสอบสิทธิ์)' : 'JWT HttpOnly Cookie (ต้องเข้าสู่ระบบ)',
          allowedRoles: isPublic ? ['ANONYMOUS', 'USER', 'ADMIN', 'SUPER_ADMIN'] : ['ADMIN', 'SUPER_ADMIN'],
          roleMatrix: {
            anonymous: isPublic,
            user: !isPublic,
            officer: !isPublic,
          ...
          responses: [{
            ...
            sample: metadata.sampleResponse,
          }],
          rateLimit: metadata.rateLimit || 'Not configured',
