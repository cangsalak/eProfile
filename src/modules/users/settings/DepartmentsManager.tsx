...
export default function DepartmentsManager() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<
    ...
    | { type: 'sub'; dept: Department; subIndex: number; subName: string }
    | null
  >(null);
  ...
  const executeDeleteSubDepartment = async (dept: Department, subIndex: number) => {
  ...
  return (
    <div className="space-y-6 pt-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {subDepts.map((sub, idx) => (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => setDeleteTarget({ type: 'sub', dept, subIndex: idx, subName: sub.name })}
                            className="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all ml-1"
                            title={`ลบ ${sub.name}`}
                          >
      ...
      <ConfirmModal
        isOpen={!!deleteTarget}
        title={deleteTarget?.type === 'dept' ? 'ยืนยันการลบหน่วยงาน?' : 'ยืนยันการลบแผนก/หมวดย่อย?'}
        message={
          deleteTarget?.type === 'dept'
            ? `คุณแน่ใจหรือไม่ที่จะลบหน่วยงาน "${deleteTarget.name}" และแผนกย่อยทั้งหมด? ข้อมูลที่ลบจะไม่สามารถกู้คืนได้`
            : `คุณแน่ใจหรือไม่ที่จะลบแผนก/หมวด "${deleteTarget?.type === 'sub' ? deleteTarget.subName : ''}"?`
        }
        ...
        onConfirm={() => {
          ...
          if (deleteTarget.type === 'dept') {
            ...
            executeDeleteSubDepartment(deleteTarget.dept, deleteTarget.subIndex);
          }
        }}
