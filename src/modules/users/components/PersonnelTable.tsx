                        ...
                        {person.currentLeave && (
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${
                              person.currentLeave.status === 'อนุมัติแล้ว'
                                ? 'bg-sky-50 text-sky-700 dark:bg-sky-950/50 dark:text-sky-300 border-sky-200 dark:border-sky-800/60'
                                : 'bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300 border-amber-200 dark:border-amber-800/60 animate-pulse'
                            }`}
                            title={
                              person.currentLeave.status === 'อนุมัติแล้ว'
                                ? `อยู่ระหว่างลา: ${person.currentLeave.leaveType} (อนุมัติแล้ว ${new Date(person.currentLeave.startDate).toLocaleDateString('th-TH')} - ${new Date(person.currentLeave.endDate).toLocaleDateString('th-TH')})`
                                : `รออนุมัติลา: ${person.currentLeave.leaveType} (${new Date(person.currentLeave.startDate).toLocaleDateString('th-TH')} - ${new Date(person.currentLeave.endDate).toLocaleDateString('th-TH')})`
                            }
                          >
                            <i className={`fa-solid ${person.currentLeave.status === 'อนุมัติแล้ว' ? 'fa-plane-departure text-[9px] text-sky-500' : 'fa-clock text-[9px] text-amber-500'}`}></i>
                            <span>{person.currentLeave.status === 'อนุมัติแล้ว' ? `ลา: ${person.currentLeave.leaveType}` : `รออนุมัติ: ${person.currentLeave.leaveType}`}</span>
                          </span>
                        )}
                      </div>
                ...
                <td className="px-6 py-3.5">
                  {person.currentLeave ? (
                    <div className="space-y-0.5">
                      {person.currentLeave.status === 'อนุมัติแล้ว' ? (
                        <>
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-sky-50 text-sky-700 dark:bg-sky-950/50 dark:text-sky-300 border border-sky-200 dark:border-sky-800/60 shadow-2xs">
                            <i className="fa-solid fa-umbrella-beach text-[11px] text-sky-500"></i>
                            <span>ลา: {person.currentLeave.leaveType}</span>
                          </span>
                          <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">
                            อนุมัติแล้ว (ถึง {new Date(person.currentLeave.endDate).toLocaleDateString('th-TH', { day: 'numeric', month: 'short' })})
                          </p>
                        </>
                      ) : (
                        <>
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60 shadow-2xs">
                            <i className="fa-solid fa-clock text-[11px] text-amber-500"></i>
                            <span>รออนุมัติ: {person.currentLeave.leaveType}</span>
                          </span>
                          <p className="text-[10px] text-amber-600 dark:text-amber-400 font-medium">
                            ยื่นขอถึง {new Date(person.currentLeave.endDate).toLocaleDateString('th-TH', { day: 'numeric', month: 'short' })}
                          </p>
                        </>
                      )}
                    </div>
                  ...
                  <div className="flex justify-end gap-2">
                    {person.currentLeave && (
                      <button
                        ...
                        className={`w-8 h-8 rounded-lg border transition-all flex items-center justify-center relative ${
                          person.currentLeave.status === 'อนุมัติแล้ว'
                            ? 'bg-sky-50 text-sky-600 hover:bg-sky-100 dark:bg-sky-950/40 dark:text-sky-400 dark:hover:bg-sky-900/50 border-sky-200 dark:border-sky-800/50'
                            : 'bg-amber-50 text-amber-600 hover:bg-amber-100 dark:bg-amber-950/40 dark:text-amber-400 dark:hover:bg-amber-900/50 border-amber-200 dark:border-amber-800/50'
                        }`}
                        title={
                          person.currentLeave.status === 'อนุมัติแล้ว'
                            ? `อยู่ระหว่างการลา (${person.currentLeave.leaveType} - อนุมัติแล้ว)`
                            : `มีรายการยื่นขอลา (${person.currentLeave.leaveType} - รออนุมัติ)`
                        }
                        aria-label={`ดูประวัติการลาของ ${person.firstName} ${person.lastName}`}
                      >
                        <i className={`fa-solid ${person.currentLeave.status === 'อนุมัติแล้ว' ? 'fa-calendar-check text-xs' : 'fa-clock text-xs'}`}></i>
                        {person.currentLeave.status === 'รออนุมัติ' && (
                          <>
                            <span className="absolute -top-1 -right-1 w-2 h-2 bg-amber-500 rounded-full border-2 border-white dark:border-slate-900 animate-ping"></span>
