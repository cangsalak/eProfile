...
import { prisma, resetPasswordWithTokenSchema } from '@/modules/core';
...
export async function handleResetPassword(req: Request) {
  try {
    ...
    await prisma.$transaction([
      ...
      prisma.auditLog.create({
        data: {
          ...
          details: JSON.stringify({ method: 'self-service-token', ip }),
          ipAddress: ip,
        },
      }),
    ]);
    ...
    return NextResponse.json({ error: 'เกิดข้อผิดพลาดในการรีเซ็ตรหัสผ่าน กรุณาลองใหม่' }, { status: 500 });
  }
}
