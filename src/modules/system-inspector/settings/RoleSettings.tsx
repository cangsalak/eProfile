...
import { ModuleRegistry, ALL_SYSTEM_MODULES } from '@/lib/modules';
...
export default function RoleSettings({ settings }: RoleSettingsProps) {
  ...
  const allPermissions = useMemo(() => {
    ...
    modulePerms.forEach(mp => {
      if (!combined.some(p => p.id === mp.key)) {
        combined.push({
          id: mp.key,
  ...
  return (
    <div className="space-y-6">
              ...
              <div key={role.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm flex flex-col group">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {!role.isSystem && (
                      <button onClick={() => setDeleteRoleId(role.id)} className="p-1.5 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg" title="ลบ">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {perms.length === 0 ? (
                      ...
                      perms.slice(0, 3).map(p => {
                        const permDef = allPermissions.find(ap => ap.id === p);
                        return (
      ...
      <ConfirmModal
        isOpen={!!deleteRoleId}
        title="ยืนยันการลบสิทธิ์ (Role)?"
        message="คุณแน่ใจหรือไม่ที่จะลบ Role นี้? ผู้ใช้งานที่ได้รับสิทธิ์นี้จะไม่สามารถเข้าถึงฟังก์ชันพิเศษได้จนกว่าจะได้รับมอบหมาย Role ใหม่"
        confirmText="ยืนยันการลบ"
        cancelText="ยกเลิก"
        isDestructive={true}
        onConfirm={() => {
          if (deleteRoleId) {
            executeDeleteRole(deleteRoleId);
          }
        }}
