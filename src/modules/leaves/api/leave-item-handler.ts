...
/**
 * Handle PUT /api/modules/leaves/[id]
 */
export async function handleUpdateLeave(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // Disallow status mutations via PUT
...
/**
 * Handle DELETE /api/modules/leaves/[id]
 */
export async function handleDeleteLeave(req: Request, { params }: { params: { id: string } }) {
  try {
    if (!isValidId(params.id)) {
      return NextResponse.json({ error: 'Invalid ID' }, { status: 400 });
    }
