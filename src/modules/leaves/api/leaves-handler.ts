...
/**
 * Handle GET /api/modules/leaves
 * - Unauthenticated -> 401
 * - Normal user     -> own leaves only
 * - APPROVE_LEAVE / MANAGE_PERSONNEL / SUPER_ADMIN -> all leaves (or filtered by personnelId)
 */
export async function handleGetLeaves(req: Request) {
...
/**
 * Handle POST /api/modules/leaves
 * - Authenticated only
 * - Normal user: forced personnelId = user.id, status = 'รออนุมัติ'
 * - MANAGE_PERSONNEL: may submit on behalf of someone else
 */
export async function handleCreateLeave(req: Request) {
  const { user, error: authError } = await requireAuth(req);
  if (authError || !user) return authError ?? NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
