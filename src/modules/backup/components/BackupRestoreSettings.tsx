...
export default function BackupRestoreSettings({ settings, isRestoring, handleRestore, restoreFileInputRef }: BackupRestoreSettingsProps) {
  ...
  return (
    <div className="space-y-6 animate-fade-in font-prompt">
      ...
      {subTab === 'backup_restore' && (
        ...
        >
          {/* Active Database Engine Banner */}
          <div className="p-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-sm text-slate-900 dark:text-white">ฐานข้อมูลระบบที่ใช้งานอยู่ (Active Database)</span>
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-primary-500/10 text-primary-600 dark:text-primary-400 border border-primary-500/20 uppercase tracking-wider">
                    {dbProvider}
                  </span>
                </div>
          ...
          <div className="p-5 sm:p-6 border border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900/50 space-y-4">
            ...
            {/* Provider-Scoped Backup Download Options */}
            <div className={`grid grid-cols-1 ${dbProvider === 'sqlite' ? 'md:grid-cols-2' : 'md:grid-cols-1'} gap-3.5 pt-1`}>
              ...
              {/* Option 2: Native SQLite (Shown ONLY if SQLite was selected) */}
              {dbProvider === 'sqlite' && (
