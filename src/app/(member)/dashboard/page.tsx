...
export default function DashboardOverviewPage() {
  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      try {
        setIsLoading(true);
        const [resStats, resSettings, resLeaves, resPosts, resNotifs, resVehicles] = await Promise.all([
        ...
        if (resVehicles && resVehicles.ok) {
          ...
          setVehiclesCount(Array.isArray(vData) ? vData.length : 0);
        }
      } catch (err) {
  ...
  return (
    <div className="pb-16 space-y-8 animate-fade-in font-prompt">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        ...
        {/* Total Personnel */}
        <Link href="/directory" className="group p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-blue-500/50 transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">กำลังพลทั้งหมด</span>
            <div className="w-9 h-9 rounded-xl bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 flex items-center justify-center text-sm group-hover:scale-110 transition-transform">
              <i className="fa-solid fa-users"></i>
            </div>
          </div>
          <div>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white font-mono">{personnelStats.total}</div>
            <div className="text-[11px] text-blue-600 dark:text-blue-400 font-medium mt-1 flex items-center gap-1">
              <span>ทำเนียบ</span> <i className="fa-solid fa-arrow-right text-[9px]"></i>
            </div>
          </div>
        </Link>

        {/* Departments */}
        <Link href="/directory" className="group p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-emerald-500/50 transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">หน่วยงาน / แผนก</span>
            <div className="w-9 h-9 rounded-xl bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-sm group-hover:scale-110 transition-transform">
              <i className="fa-solid fa-sitemap"></i>
            </div>
          </div>
          <div>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white font-mono">{personnelStats.departments}</div>
            <div className="text-[11px] text-emerald-600 dark:text-emerald-400 font-medium mt-1 flex items-center gap-1">
              <span>โครงสร้าง</span> <i className="fa-solid fa-arrow-right text-[9px]"></i>
            </div>
          </div>
        </Link>

        {/* Active Leaves */}
        <Link href="/leave" className="group p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-amber-500/50 transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">กำลังพลลาวันนี้</span>
            <div className="w-9 h-9 rounded-xl bg-amber-50 dark:bg-amber-950/50 text-amber-600 dark:text-amber-400 flex items-center justify-center text-sm group-hover:scale-110 transition-transform">
              <i className="fa-solid fa-plane-departure"></i>
            </div>
          </div>
          <div>
            <div className="text-2xl sm:text-3xl font-black text-amber-600 dark:text-amber-400 font-mono">{activeLeavesToday.length}</div>
            <div className="text-[11px] text-amber-600 dark:text-amber-400 font-medium mt-1 flex items-center gap-1">
              <span>ระบบการลา</span> <i className="fa-solid fa-arrow-right text-[9px]"></i>
            </div>
          </div>
        </Link>

        {/* Vehicles */}
        <Link href="/profile" className="group p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-purple-500/50 transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">ยานพาหนะ</span>
            <div className="w-9 h-9 rounded-xl bg-purple-50 dark:bg-purple-950/50 text-purple-600 dark:text-purple-400 flex items-center justify-center text-sm group-hover:scale-110 transition-transform">
              <i className="fa-solid fa-car-side"></i>
            </div>
          </div>
          <div>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white font-mono">{vehiclesCount}</div>
            <div className="text-[11px] text-purple-600 dark:text-purple-400 font-medium mt-1 flex items-center gap-1">
              <span>ลงทะเบียน</span> <i className="fa-solid fa-arrow-right text-[9px]"></i>
            </div>
          </div>
        </Link>

        {/* Notifications */}
        <Link href="/manage/notifications" className="group p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-rose-500/50 transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">การแจ้งเตือน</span>
            <div className="w-9 h-9 rounded-xl bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400 flex items-center justify-center text-sm group-hover:scale-110 transition-transform">
              <i className="fa-solid fa-bullhorn"></i>
            </div>
          </div>
          <div>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white font-mono">{recentNotifs.length}</div>
            <div className="text-[11px] text-rose-600 dark:text-rose-400 font-medium mt-1 flex items-center gap-1">
              <span>กระจายข่าว</span> <i className="fa-solid fa-arrow-right text-[9px]"></i>
            </div>
          </div>
        </Link>

        {/* News & Posts */}
        <Link href="/news" className="group p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-indigo-500/50 transition-all flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">ข่าวสาร & บทความ</span>
            <div className="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-sm group-hover:scale-110 transition-transform">
              <i className="fa-solid fa-newspaper"></i>
            </div>
          </div>
          <div>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white font-mono">{recentPosts.length}</div>
            <div className="text-[11px] text-indigo-600 dark:text-indigo-400 font-medium mt-1 flex items-center gap-1">
              <span>ประชาสัมพันธ์</span> <i className="fa-solid fa-arrow-right text-[9px]"></i>
            </div>
          </div>
        </Link>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 sm:p-7 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 pt-2">
          ...
          <Link
            ...
            className="group p-4 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-indigo-500/50 bg-slate-50/50 dark:bg-slate-800/30 hover:bg-white dark:hover:bg-slate-800/80 transition-all flex items-start gap-4"
          >
            <div className="w-11 h-11 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-lg shrink-0 group-hover:scale-105 transition-transform">
              <i className="fa-solid fa-bullhorn"></i>
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/40">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-sm font-bold">
              ...
              {recentPosts.length === 0 ? (
                ...
                recentPosts.map((post) => (
                  <Link
                    key={post.id}
                    href={`/news/${post.id}`}
                    className="group block p-3 rounded-2xl border border-slate-100 dark:border-slate-800/80 hover:border-indigo-500/40 hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-all"
                  >
          ...
          <div className="p-5 rounded-3xl bg-gradient-to-br from-slate-900 to-indigo-950 text-white shadow-lg space-y-4 border border-slate-800">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-xs font-bold text-white">ความปลอดภัยและสถานะระบบ</div>
                  <div className="text-[10px] text-slate-300">eProfile Platform v1.2.0</div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="p-2.5 rounded-xl bg-white/5 border border-white/10">
                <div className="text-[10px] text-slate-400">สิทธิ์ผู้ใช้งาน</div>
                <div className="font-bold text-white mt-0.5">{currentUser.role}</div>
              </div>
              <div className="p-2.5 rounded-xl bg-white/5 border border-white/10">
                <div className="text-[10px] text-slate-400">การสำรองข้อมูล</div>
                <div className="font-bold text-emerald-300 mt-0.5">พร้อมใช้งาน</div>
              </div>
            </div>

            {isAdmin && (
              <div className="pt-1 flex items-center justify-between text-xs border-t border-white/10">
                <Link href="/manage/inspector" className="text-indigo-300 hover:text-white flex items-center gap-1 font-semibold text-[11px] transition-colors">
                  <i className="fa-solid fa-wand-magic-sparkles"></i> ตรวจสอบระบบ (Inspector)
                </Link>
                <Link href="/manage/api-docs" className="text-indigo-300 hover:text-white flex items-center gap-1 font-semibold text-[11px] transition-colors">
