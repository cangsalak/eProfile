...
export default function ManageMediaPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('ยืนยันการลบไฟล์นี้? (การลบจะลบข้อมูลจากฐานข้อมูลเท่านั้น ไม่ได้ลบไฟล์จริงในขณะนี้)')) return;
    try {
              ...
              <div key={file.id} className="group relative border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-800/50">
                ...
                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    className="w-8 h-8 rounded-lg bg-red-500 text-white hover:bg-red-600 shadow-md flex items-center justify-center"
                  >
