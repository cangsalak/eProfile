...
export default function SystemInspectorPage() {
  ...
  const updateFindingStatus = async (
    inspectionId: string,
    findingId: string,
    newStatus: 'OPEN' | 'REVIEWED' | 'IGNORED' | 'FIXED' | 'FALSE_POSITIVE'
  ) => {
    try {
  ...
  const deleteInspection = async (id: string) => {
    if (!confirm('ยืนยันการลบประวัติการตรวจสอบนี้?')) return;
    try {
