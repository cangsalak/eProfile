 ...
 * 
 * Atomically rejects a pending leave request with strict reason validation,
 * role-scoping, self-approval prevention, audit logging, and notification creation.
 */
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!isGlobalViewer) {
      if (user.role === 'DEPARTMENT_COMMANDER') {
        if (leave.personnel.department !== userDept) {
          return NextResponse.json(
            { error: 'ไม่มีสิทธิ์ไม่อนุมัติใบลาของกำลังพลนอกหน่วยงานที่รับผิดชอบ' },
            { status: 403 }
          );
        }
      } else {
        ...
        if (
          ...
          return NextResponse.json(
            { error: 'ไม่มีสิทธิ์ไม่อนุมัติใบลาของกำลังพลนอกหน่วยย่อยที่รับผิดชอบ' },
            { status: 403 }
          );
        }
      }
    ...
    if (isSelfApproval && user.role !== 'SUPER_ADMIN') {
      return NextResponse.json(
        { error: 'ไม่อนุญาตให้ดำเนินการใบลาของตนเอง' },
        { status: 403 }
      );
    }
    ...
    try {
      await prisma.notification.create({
        data: {
          ...
          title: `ใบลาไม่ได้รับการอนุมัติ (${leave.leaveType})`,
          message: `ใบลาช่วงวันที่ ${formattedStartDate} - ${formattedEndDate} ไม่ได้รับการอนุมัติ เนื่องจาก: ${reason}`,
          type: 'error',
          link: '/leave',
          isRead: false,
        },
      ...
      console.error('Failed to create leave rejection notification:', notifErr);
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'บันทึกการไม่อนุมัติใบลาเรียบร้อยแล้ว',
      data: {
        id: params.id,
        status: 'ไม่อนุมัติ',
