...
if [ -n "$DATABASE_URL" ] && echo "$DATABASE_URL" | grep -q "^file:"; then
  ...
  if [ -f "/app/node_modules/prisma/build/index.js" ]; then
    ...
    node /app/node_modules/prisma/build/index.js db push --schema=/app/prisma/schema.prisma --skip-generate 2>/dev/null || true
  ...
  if echo "$DATABASE_URL" | grep -q "^mysql:"; then
    node /app/node_modules/prisma/build/index.js db push --schema=/app/prisma/schema.mysql.prisma --skip-generate 2>/dev/null || true
  elif echo "$DATABASE_URL" | grep -qE "^postgres(ql)?:"; then
    node /app/node_modules/prisma/build/index.js db push --schema=/app/prisma/schema.postgresql.prisma --skip-generate 2>/dev/null || true
  fi
fi
